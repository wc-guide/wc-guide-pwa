<template>
    <div class="app" id="app">
        ...
        <div class="navigation-mobile app__mobilenav">
            <button class="navtoggler navigation-mobile__navtoggler" aria-controls="mobile-nav" aria-expanded="false" @click="toggleMobileNav" id="mobile-navtoggler">
                <span v-for="line in 3" :key="line" :class="`navtoggler__line navtoggler__line--${line}`"></span>
            </button>
            <nav class="navigation-mobile__navigation navigation" id="mobile-nav" aria-hidden="true">
                <localized-link v-for="(icon, element) in navigation" :key="element" active-class="navigation-element--active" :to="`/${element}/`" :class="`navigation-element navigation-element--${element}`">
...
const Main = () =>
    import(/* webpackChunkName: "map" */ "./components/Main.vue");
