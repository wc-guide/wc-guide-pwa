<template>
	...
	>
		...
		<template v-if="toAgree">
			...
			<hello-input
				:title="$t('form_agreement')"
				name="toilet-form-agreement"
				type="checkbox"
				class="o-helloform__element--black"
				:hint="function(){setPrivacy(true)}"
			></hello-input>
		</template>
		<template v-else>
			<slot></slot>
			...
			<hello-input
				...
				:hellochange="lnglatUpdate"
			></hello-input>
			<hello-input
				title="entry Lng"
				name="toilet-lng"
				type="hidden"
				:value="defaultValues.lng"
				:hellochange="lnglatUpdate"
			></hello-input>
			<hello-input
				:title="$t('form_toilet_type')"
				type="select"
				name="toilet-toilet_type_id"
				:required="true"
				:choices="toilettypes"
				:value="defaultValues.toilet_type_id"
			...
			></hello-input>
			<hello-input
				:title="$t('form_address') + ', ' + $t('form_place')"
				name="toilet-addressplace"
				:value="defaultValues.addressplace"
				:required="true"
				:disabled="disabled || inputdisabled"
			></hello-input>
			<hello-input
			...
			></hello-input>
			<hello-input
				title="entry Country Code"
				name="toilet-country_code"
				type="hidden"
				:value="defaultValues.country_code"
			></hello-input>
			<hello-input
				:title="$t('form_address')"
				name="toilet-address"
				type="hidden"
			...
			></hello-input>
			<div
				:class="'o-helloform__element '+((disabled||inputdisabled)?'o-helloform__element--disabled':'')"
			>
			...
			<hello-input
				v-for="(prop, propId) in properties"
				:key="prop"
				:title="$t(`form_property_${prop}`) + `<img class='o-helloform__label-icon' src='/assets/img/marker/${prop}.svg'/>`"
				:name="`toilet-details-${prop}`"
				:value="propId"
				type="checkbox"
				class="o-helloform__element--black"
				:checked="defaultValues.details.indexOf(propId) !== -1"
				:disabled="disabled || inputdisabled"
			></hello-input>
			<hello-input
				:title="$t('form_comments')"
				name="toilet-comment"
				type="textarea"
...
<script>
	...
	import {toilet, openMapPopup, getEntryDescription} from "./../../vendor/funcs";
	...
	export default {
		...
		methods: {
			submit: function (data) {
				if (data['toilet-lat'] === '0') {
					...
					if (this.map) {
						...
						openMapPopup(getEntryDescription(entry), coordinates, this.map);
						let zoom = 14;
					...
					if (data['toilet-form-agreement']) {
						...
						this.submitToilet(this.toAgree)
							...
							.catch(error => {
								this.loading = false;
								this.error = this.$t('form_error') + (error ? `: ${error}` : '');
							});
					} else {
			...
			lnglatUpdate: function () {
				...
				const lat = $lat.value;
				if (lng !== "0" && lat !== "0") {
					const url = searchPlace.replace('{lat}', lat).replace('{lng}', lng);
					axios.get(url).then(resp => {
						...
						this.$el.querySelector('[name=toilet-addressplace]').value = `${strasse} ${nummer}, ${ort}`;
						this.$el.querySelector('[name=toilet-address]').value = `${strasse} ${nummer}`;
						this.$el.querySelector('[name=toilet-place]').value = `${ort}`;
						this.$el.querySelector('[name=toilet-country]').value = country;
						this.$el.querySelector('[name=toilet-country_code]').value = country_code;
					});
				}
			},
